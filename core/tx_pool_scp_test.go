...
func TestDOTxScriptSolt(t *testing.T) {
	...
	amount, _ := new(big.Int).SetString("20000749999999997500000", 10)
	tx := types.NewTransaction(1, common.HexToAddress("0x80a5A916FB355A8758f0a3e47891dc288DAC2665"), amount, 250000, new(big.Int).SetInt64(100000000001), common.Hex2Bytes(data))
	DOTxScript(*tx)
}
...
func TestSendTx(t *testing.T) {
	...
	hash, err := SendTx(
		...
		*tx,
		...
		coin1,
		coin2,
		new(big.Int).SetInt64(1111), new(big.Int).SetInt64(2*1e18), new(big.Int).SetInt64(2.6666*1e18), new(big.Int).SetInt64(680))
	if err != nil {
		panic(err)
	}
	t.Logf("hash %v", hash)
